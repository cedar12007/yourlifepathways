...
return;$p=$e.data(key);$e.insertAfter($p);$e.removeData(key);}});};})(jQuery);$(document).ready(function(){function updateNav(){var path=window.location.pathname;var hash=window.location.hash;var $navLinks=$('#nav a');$navLinks.removeClass('active');if(path.includes('blog')){$navLinks.filter('[href*="blog"]').addClass('active');}
else if(hash){$navLinks.filter('[href*="'+hash+'"]').addClass('active');}
else if(path==='/'||path.includes('index')){$navLinks.first().addClass('active');}}
updateNav();$(window).on('hashchange',updateNav);});