...
$(document).ready(function () {
	...
	if (window.location.pathname.includes('blog')) {
		...
		return; // Stop here, no need for scrollspy on blog pages
	}

	// 2. Homepage Scrollspy Logic (only runs if not on blog)

	// Default to first link (Overview)
	$navLinks.first().addClass('active');

	// Section Tracking Variables
	var trackedSectionId = null;
	...
	// Trigger initial scroll to set active based on current position
	$(window).trigger('scroll');

	// Scrolly for smooth scrolling (already imported in base.html)
	$('.scrolly').scrolly({
		speed: 1000,
		offset: 130
	});
	...
	// Track anchor link clicks for traffic logs
	$('a[href^="#"]').on('click', function () {
		var href = $(this).attr('href');
		if (href !== '#') {
			var fullUrl = window.location.origin + window.location.pathname + href;
			fetch('/log_event?url=' + encodeURIComponent(fullUrl) + '&_cb=' + Date.now());
		}
	});

	// Initial page load ping (captures visits even if page is edge-cached)
