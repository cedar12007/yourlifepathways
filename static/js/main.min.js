breakpoints({xlarge:['1281px','1680px'],large:['981px','1280px'],medium:['737px','980px'],small:['481px','736px'],xsmall:[null,'480px']});(function($){$.fn.navList=function(){var $this=$(this);$a=$this.find('a'),b=[];$a.each(function(){var $this=$(this),indent=Math.max(0,$this.parents('li').length-1),href=$this.attr('href'),target=$this.attr('target');b.push('<a '+
...
return;$p=$e.data(key);$e.insertAfter($p);$e.removeData(key);}});};})(jQuery);$(document).ready(function(){var $window=$(window),$body=$('body'),$main=$('#main'),$nav=$('#nav'),$navLinks=$nav.find('a'),$navButton;if(window.location.pathname.includes('blog')){$navLinks.removeClass('active');$navLinks.filter('[href*="blog"]').addClass('active');$navButton=$('<div id="navButton">'+
'<a href="#navPanel" class="toggle"></a>'+
'</div>').appendTo($body);$('<button id="navPanelClose"></button>').appendTo($body).on('click',function(event){event.preventDefault();event.stopPropagation();$body.removeClass('is-navPanel-visible');});$('<div id="navPanel">'+
'<nav>'+
$('#nav').navList()+
'</nav>'+
'</div>').appendTo($body).panel({delay:500,hideOnClick:true,hideOnSwipe:true,hideOnEscape:true,resetScroll:true,resetForms:true,side:'left',target:$body,visibleClass:'is-navPanel-visible'});breakpoints.on('<=medium',function(){$navButton.show();});breakpoints.on('>medium',function(){$navButton.hide();});return;}
$navLinks.first().addClass('active');var trackedSectionId=null;var sectionTimer=null;$(window).on('scroll',function(){var scrollTop=$(window).scrollTop()+130;var currentSection=null;var $sections=$main.children('section');$sections.each(function(){var $section=$(this);var sectionTop=$section.offset().top-130;var sectionBottom=sectionTop+$section.outerHeight();if(scrollTop>=sectionTop&&scrollTop<sectionBottom){currentSection=$section;return false;}});if(!currentSection){var $lastSection=$sections.last();if(scrollTop>=$lastSection.offset().top-130){currentSection=$lastSection;}}
$navLinks.removeClass('active');if(currentSection){var id=currentSection.attr('id');$navLinks.each(function(){var href=$(this).attr('href');if(href&&href.includes('#'+id)){$(this).addClass('active');}});if(id!==trackedSectionId){if(sectionTimer)clearTimeout(sectionTimer);sectionTimer=setTimeout(function(){trackedSectionId=id;var fullUrl=window.location.origin+window.location.pathname+'#'+id;console.log("Logging section view:",fullUrl);fetch('/log_event?url='+encodeURIComponent(fullUrl)+'&_cb='+Date.now());},500);}}});$(window).trigger('scroll');$('.scrolly').scrolly({speed:1000,offset:130});$('<div id="navPanel">'+
'<nav>'+
$('#nav').navList()+
'</nav>'+
'</div>').appendTo($body).panel({delay:500,hideOnClick:true,hideOnSwipe:true,hideOnEscape:true,resetScroll:true,resetForms:true,side:'left',target:$body,visibleClass:'is-navPanel-visible'});$navButton=$('<div id="navButton">'+
'<a href="#navPanel" class="toggle"></a>'+
'</div>').appendTo($body);$('<button id="navPanelClose"></button>').appendTo($body).on('click',function(event){event.preventDefault();event.stopPropagation();$body.removeClass('is-navPanel-visible');});breakpoints.on('<=medium',function(){$navButton.show();});breakpoints.on('>medium',function(){$navButton.hide();});$('a[href^="#"]').on('click',function(){var href=$(this).attr('href');if(href!=='#'){var fullUrl=window.location.origin+window.location.pathname+href;fetch('/log_event?url='+encodeURIComponent(fullUrl)+'&_cb='+Date.now());}});fetch('/log_event?url='+encodeURIComponent(window.location.href)+'&_cb='+Date.now());});